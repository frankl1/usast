...
from numba import njit, prange, float32 as T, int32, int32, optional
...
@njit(nb.types.UniTuple(T, 3)(T[:], T[:], T[:], T[:], optional(T)), fastmath=False)
def apply_kernel(ts, ts_err, arr, arr_err, r):
...
@njit(nb.types.UniTuple(T[:, :], 2)(T[:,:], T[:,:], T[:,:], T[:, :], T), parallel=True, fastmath=False)  
def apply_kernels(X, X_err, kernels, kernels_err, r=-1.0):
    ...
    for i in prange(n_kernels):
        ...
        for t in range(n_X):
            ...
            if r < 0:
                out[t, i], out_err[t, i] = apply_kernel(ts, ts_err, k, k_err, r)[:2]
            else:
                out[t, i], out_err[t, i], out[t, n_kernels + i] = apply_kernel(ts, ts_err, k, k_err, r)
...
class USAST(BaseEstimator, ClassifierMixin):
    ...
    def __init__(self, cand_length_list, shp_step = 1, nb_inst_per_class = 1, use_count=True, random_state = None, classifier = None, radius=0.0, drop_duplicates = True, scale_before_fit = True):
    ...
    def init_usast(self, X, X_err, y):
        ...
        self.kernels_err_dict = {i: np.zeros((len(range(0, m - i + 1, self.shp_step)) * n, i), dtype=np.float32) for i in self.cand_length_list}
    ...
    def transform(self, X, X_err):
        ...
        for k in self.kernels_dict.keys():
            X_transformed, X_transformed_err = apply_kernels(X, X_err, self.kernels_dict[k], self.kernels_err_dict[k], self.radius if self.use_count else -1.0)
    ...
    def predict(self, X, X_err):
        ...
        if self.scale_before_fit:
            X_transformed_concat = self.scaler.transform(X_transformed_concat)

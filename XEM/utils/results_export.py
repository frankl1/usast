...
def export_results(clf, encoder, configuration, X_train, y_train, X_validation, 
    ...
    precision_train, recall_train, f1_train, _ = precision_recall_fscore_support(res_train_mts.target_num, 
                                    res_train_mts.pred_num.astype(int), average='weighted')
    ...
    if configuration['validation_split'][1] != 0:
        ...
        precision_validation, recall_validation, f1_validation, _ = precision_recall_fscore_support(res_validation_mts.target_num, 
                                             res_validation_mts.pred_num.astype(int), average='weighted')
    ...
    precision_test, recall_test, f1_test, _ = precision_recall_fscore_support(res_test_mts.target_num, 
                                   res_test_mts.pred_num.astype(int), average='weighted')
    ...
    log('\nPrecision train: {0}'.format(precision_train))
    log('Precision validation: {0}'.format(precision_validation))
    log('Precision test: {0}'.format(precision_test))
    log('\nRecall train: {0}'.format(recall_train))
    log('Recall validation: {0}'.format(recall_validation))
    log('Recall test: {0}'.format(recall_test))
    log('\nF1 train: {0}'.format(f1_train))
    log('F1 validation: {0}'.format(f1_validation))
    ...
    results_export.columns = ['Dataset', 'Experiment', 'Window_Percentage', 'Trees', 
                              ...
                              'Accuracy_Validation','Accuracy_Test', 'Precision_Train', 'Precision_Validation', 'Precision_Test',
                              'Recall_Train', 'Recall_Validation', 'Recall_Test', 'F1_Train', 'F1_Validation', 'F1_Test']
